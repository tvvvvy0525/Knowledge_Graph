...
public class VectorStoreLoader implements CommandLineRunner {
    ...
    public VectorStoreLoader(VectorStore vectorStore, Neo4jClient neo4jClient) {
    ...
    public void run(String... args) {
        try {
            logger.info("🧹 正在清理旧的向量数据...");
            neo4jClient.query("MATCH (n:Document) DETACH DELETE n")
                    .run();

            logger.info("✅ 旧数据清理完毕，开始加载新数据...");
            // 1. 读取 JSON
